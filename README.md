...
### • Burada “-p” parametresi ile eger “deneme” klasörü yoksa hata vermez, olusturur ve onun da icine “test” klasörü olusturur
```bash
mkdir –p /home/deneme/test
```
...
o cd test\dosya		-->	adi "test dosya" olan klasöre gecmek icin kullanilir. "\" isareti bosluk(space) anlamindadir
```
...
o rmdir -p test/dene	-->	(parents) ic ice klasör silmek icin kullanilir
```
...
o mkdir -p test/dene		-->	(parents) ic ice klasör olusturmak icin kullanilir
o mkdir -m 444 test		-->	(mode) test adinda sadece yazma izinleri olan bir klasör olusturur
```

### • Istenilen uzantiya sahip dosya olusturmak icin kullanilir
```bash
touch test.txt test.yml
```
```bash
...
```
```bash
o mv test.doc n/can.doc		-->	Dosyayi bir yere tasimak icin kullanilir. Istersek ismini tasima sirasinda degistirebiliriz
```

### • Bir dosyayi bir yerden baska bir yere kopyalamak icin kullanilir. Istersek kopyalama sirasinda isim degistirebiliriz
```bash
cp test.doc home/ali.doc
```
...
### • (changemode) Dosya izinlerini degistirmek icin kullanilir. Burada "test" dosyasina calisma izni verir
```bash
chmod +x test.sh
```
```bash
...
```
```bash
o chmod -x test.sh		-->	burada tüm kullanicilarin calistirma iznini alir	
o chmod +rw test.sh		-->	tüm kullanicilar icin okuma ve yazma izni verir
o chmod 777 test.sh		-->	burada dosyaya tüm kullanicilar icin bütün izinleri verir 
o chmod ugo=rwx	test.sh		--> 	u >> user, g >> group, o >> other.  Tüm kullanicilar icin bütün izinleri verir 
o chmod ug=r test.sh		--> 	dosyaya sadece user ve group icin okuma izni verir 
```
...
```bash
echo test | xargs –I {} echo {} {} 	# büyük i
```
...
envsubst < test.txt > ali.txt		-->	var olan bir dosyanin sonuna ekleme yapmak icin ">>" kullanilabilir
ali.txt		>> selam kanka
```
...
```bash
sudo useradd -m -d /home/test -c "test negiz" test
```
```bash
...
```bash
sudo usermod –c “aws solution archtitect” test
```

### • “test” kullanici adini “Murat” olarak degistirir
```bash
sudo usermod –l murat test
```
...
### • “Aws” grubuna test kullanicisini ekler
```bash
sudo gpasswd –a test aws
```

### • “Aws” grubundan “test” kullanicisini siler
```bash
sudo gpasswd –d test aws
```
...
```bash
grep naber -A 3 clarusway test.txt
```
```bash
...
o cat test.txt | tr –d aeiou		-->	“-d” sil anlamindadir. test dosyasi icindeki belirli harfleri silmek icin kullanilir. Sadece cikti olarak gösterir, ana dosya etkilenmez
o cat test.txt | tr [a-z] [A-Z]		-->	Dosya icerisindeki bütün harfleri büyük olacak sekilde yazdirir. Dosya etkilenmez, sadece ciktida degisir
o cat test.txt | tr [:space:] '\n'	-->	Tüm bosluklardan sonra “enter” eklemek icin kullanilir
```

### • test dosyasi icindeki satir, kelime ve karakter olarak sayisini gösterir
```bash
wc test.txt
```
```bash
...
```bash
rm test.txt && echo 'silindi' || 'hata'
```

### • “#” isareti ile komut sonuna yorum satiri eklenebilir
```bash
echo test # burasi yorum satiridir
```
...
```bash
#!/bin/bash
if [ -z "$(ls | grep test)" ]; then	# “-z” bos ise anlamindadir.
        mkdir test
        echo "test klasörü olusturuldu"
else
